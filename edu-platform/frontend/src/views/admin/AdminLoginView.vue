...
async function login(){if(!await formRef.value.validate().catch(()=>false))return;loading.value=true;try{await store.login({account:form.account,password:form.password},form.remember);if(!store.isAdmin){await store.logout('/admin/login');ElMessage.error('该账号没有管理员权限');return}router.replace('/admin/dashboard')}finally{loading.value=false}}
</script>
<style scoped>
