...
public class LearningAgentController {
  ...
  @PostMapping("/plans/{id}/confirm") public Result<Map<String,Object>> confirm(@PathVariable("id") String id){return Result.success(service.confirm(user(),id));}
  @GetMapping("/plans/current") public Result<Map<String,Object>> current(){return Result.success(service.current(user()));}
  @PostMapping("/tasks/{id}/complete") public Result<Map<String,Object>> complete(@PathVariable("id") String id){return Result.success(service.changeTask(user(),id,true));}
  @PostMapping("/tasks/{id}/reopen") public Result<Map<String,Object>> reopen(@PathVariable("id") String id){return Result.success(service.changeTask(user(),id,false));}
  ...
  @GetMapping("/conversations/{id}/messages") public Result<?> history(@PathVariable("id") String id){return Result.success(chatService.history(user(),id));}
